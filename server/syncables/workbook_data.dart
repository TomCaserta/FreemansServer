    ...
    Future.wait(futures).then((List<bool> returnVal) {
      ...
      if (all) {
        if (this.isNew) {
          ...
          dbh.prepareExecute("INSERT INTO `produce` (`productID`, `supplierID`, `haulageID`, `cost`, `weightID`, `packagingID`, `timeofpurchase`, `insertedBy`, `active`) VALUES (?,?,?,?,?,?,?,?,?)", [product.product.ID, supplierID, haulierID, cost, product.item.ID, product.packaging.ID, purchaseTime.millisecondsSinceEpoch, null, (isActive ? 1 : 0)]).then((Results res) {
            if (res.insertId != 0) {
              this._firstInsert(res.insertId);
              c.complete(true);
              ffpServerLog.info("Created new ${this.runtimeType}");
          ...
          dbh.prepareExecute("UPDATE `produce` SET `productID`=?, `supplierID`=?, `haulageID`=?, `cost`=?, `weightID`=?, `packagingID`=?, `timeofpurchase`=?, `insertedBy`=?, `active`=? WHERE ID=?", [product.product.ID, supplierID, haulierID, cost, product.item.ID, product.packaging.ID, purchaseTime.millisecondsSinceEpoch, null, (isActive ? 1 : 0), ID]).then((res) {
            if (res.affectedRows <= 1) {
              this.synced();
              c.complete(true);
            } else {
